...
export default {
  ...
  generator: {
    ...
    barcodeErrors: {
      ...
      invalidFormat: '格式不正确',
      tooShort: '最少需要{{min}}个字符（当前: {{current}}个）',
      tooLong: '最多允许{{max}}个字符（当前: {{current}}个）',
      unknownFormat: '未知的条形码格式',
      evenDigits: '请输入偶数位数',
      needsEvenLength: '需要偶数位数（当前: {{current}}位）',
      invalidNumber: '只能输入数字',
      valueTooSmall: '最小值为{{min}}',
      valueTooLarge: '最大值为{{max}}',
      pharmacodeRange: '请输入3到131070之间的数字',
      invalid: '格式不正确',
      // bwip-js错误消息翻译
      onlyDigits: '只能输入数字',
      onlyDigitsAndUppercase: '只能输入数字和大写字母',
      code93Chars: '只能输入数字、大写字母、空格和符号(-.$+/)',
      code39Chars: '只能输入数字、大写字母、空格和符号(-.$/+%)',
      code128Chars: '只能输入ASCII字符',
      invalidChar: '包含无效字符',
      invalidLength: '输入长度不正确',
      invalidCheckDigit: '校验位不正确',
      mustBeEven: '请输入偶数位数字',
      tooLongData: '输入数据太长',
      tooShortData: '输入数据太短',
      invalidStartStop: '起始/结束字符不正确',
      outOfRange: '值超出范围',
    },
    types: {
      website: '网站',
